...
This gate never treats updated_at as the source NAV date and never fabricates a date.
It uses the latest ingest run_id (run_YYYYMMDD_HHMMSS) in nav_staging to determine
whether a fund was actually extracted/matched during the current ingest.
"""
...
def main():
    ...
    official = get(
        "nav_official",
        select="fund_id,nav,as_of_date,source_id,source_url,verified_at,updated_at",
        limit="5000",
    )
    staging = get(
        "nav_staging",
    ...
    for r in current_rows:
        fid = r.get("fund_id")
        sid = r.get("source_id")
        if fid and r.get("match_status") == "matched":
            current_by_fund.setdefault(fid, []).append(r)
        if sid:
            current_by_source[sid] = current_by_source.get(sid, 0) + 1
    ...
    for f in funds:
        ...
        if host not in SUPPORTED:
            ...
            status = "FAIL_NO_NAV"
            failures.append(f"{f['canonical_name']} [{host}] no official NAV")
        elif not d:
            status = "FAIL_NO_DATE"
            failures.append(f"{f['canonical_name']} [{host}] official NAV without as_of_date")
        elif d > today:
            status = "FAIL_FUTURE_DATE"
            failures.append(f"{f['canonical_name']} [{host}] as_of={d} > {today}")
        elif candidates:
            if candidate_date and candidate_date > today:
                status = "FAIL_FUTURE_CANDIDATE"
                failures.append(f"{f['canonical_name']} [{host}] current-run candidate as_of={candidate_date} > {today}")
    ...
    out = {
        ...
        "current_run_sources": current_by_source,
        "funds": len(funds),
        "counts": counts,
        "rows": rows,
    }
    ...
    # Hard-fail only on structural/data-integrity violations. Source lag and a
    # legitimate older published NAV are reported, not treated as pipeline errors.
    if any(r["status"].startswith("FAIL_") for r in rows):
        sys.exit(1)
