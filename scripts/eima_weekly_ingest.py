...
def parse_tables_page(page, pno: int, rows: list, warnings: list) -> tuple:
...
def parse_layout_text_page(text: str, pno: int, rows: list, warnings: list, known_names: set) -> int:
...
def parse_pdf(blob: bytes, known_fund_names=None):
...
def load_known_fund_names() -> set:
    funds = sb_get("funds", {"select": "fund_id,canonical_name,eima_name_raw,active", "limit": "1000"})
    names = set()
    for f in funds:
        ...
        for n in (f.get("canonical_name"), f.get("eima_name_raw")):
            if n and n.strip():
                names.add(n.strip())
    try:
        aliases = sb_get("fund_name_aliases", {"select": "alias_name", "limit": "5000"})
        for a in aliases:
            if a.get("alias_name"):
...
def previous_report_fund_ids(before_date: str) -> set:
    reps = sb_get(
        "eima_reports",
        {"select": "report_id,report_date", "report_date": f"lt.{before_date}", "order": "report_date.desc", "limit": "1"},
    )
    if not reps:
        return set()
    ...
    while True:
        batch = sb_get(
            ...
            {"select": "fund_id", "report_date": f"eq.{prev}", "limit": "1000", "offset": str(offset)},
        )
        if not batch:
            break
        for row in batch:
...
def match_rows(rows):
    funds = sb_get(
        "funds",
        {"select": "fund_id,canonical_name,eima_name_raw,management_company,currency,active", "limit": "1000"},
    )
    aliases = sb_get(
        "fund_name_aliases",
    ...
    for r in rows:
        ...
        if not f:
            ...
            continue
        r.update({"fund_id": f["fund_id"], "currency": f.get("currency") or "EGP", "match_score": score})
        matched.append(r)
...
def main():
    ...
    existing = sb_get(
        "eima_reports",
        {"select": "report_id,report_date,source_sha256", "or": f"(report_date.eq.{report_date},source_sha256.eq.{sha})"},
    )
    ...
    payload = {
        ...
        "page_count": len(pages),
        "parser_version": PARSER_VERSION,
        "notes": json.dumps({"run_id": run_id, "page_audit": pages, "coverage": coverage_meta, "layout_only_funds": layout_only}),
    }
    dbrows = [
        {
            "fund_id": r["fund_id"],
