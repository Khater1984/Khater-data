#!/usr/bin/env python3
"""Recover PFI NAVs from the manager's current funds page.

The page currently exposes NAV/date as rendered text rather than the HTML
-table shape expected by the legacy scraper. This repair is deliberately
source-specific and writes only values/date pairs present on the official page.
"""
...
def post(path,payload):
 r=requests.post(f"{BASE}/rest/v1/{path}",headers={**H,"Prefer":"return=minimal"},json=payload,timeout=30); r.raise_for_status()
def main():
 ...
 funds=get('funds',select='fund_id,canonical_name',active='eq.true',limit='1000')
 by={f['canonical_name']:f for f in funds}; rows=[]
 ...
 for r in rows: post('nav_staging',r); post('nav_official',{'fund_id':r['fund_id'],'nav':r['nav'],'currency':'EGP','as_of_date':r['as_of_date'],'source_id':SID,'source_url':URL,'verified_at':datetime.now(timezone.utc).isoformat()})
 print(f'PFI repair: {len(rows)} NAVs promoted from official source')
