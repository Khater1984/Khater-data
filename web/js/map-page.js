...
const META={
 ...
 tbill:{text:"عائد مزاد أذون 91 يوماً يُطبَّق زمنياً. أول مزاد في السلسلة قد يكون بعد يناير 2016 ويُذكر على البطاقة.",href:"https://www.cbe.org.eg/en/economic-research/statistics/interest-rates",link:"أذون الخزانة — البنك المركزي"}
};
...
function lastOf(key){const rows=state.series?.[key]?.rows||[];return rows.length?rows[rows.length-1]:null}
function path(key){const rows=state.series?.[key]?.rows||[];if(!rows.length)return[];const from=state.range==="all"?START:state.range==="5"?"2021-01-01":"2024-01-01";const sliced=rows.filter(r=>r.time>=from);if(key==="purchasing"||key==="deposit"||key==="tbill")return toLine(sliced);return rebase(sliced)}
