...
(function(){
  ...
  window.benchmark=async function(k,h,end){
    ...
    else if(h==='last12m') d.setUTCFullYear(d.getUTCFullYear()-1);
    else if(h==='ytd') d.setUTCMonth(0,1);
    else if(/^\dy$/.test(h)) d.setUTCFullYear(d.getUTCFullYear()-Number(h[0]));
    else return null;
    ...
    const v=r.filter(x=>x.value!=null);
    if(!v.length) return null;
    if(b.type==='level'){
      const a=Number(v[0].value),z=Number(v[v.length-1].value);
      if(!Number.isFinite(a)||a===0||!Number.isFinite(z)) return null;
      return {value:(z/a-1)*100,start,end,unit:v[v.length-1].unit,calculated:true};
    }
    if(b.type==='inflation'){
      let f=1;v.forEach(x=>{f*=1+Number(x.value)/100});
      return {value:(f-1)*100,start,end,unit:v[v.length-1].unit,calculated:true};
    }
    if(b.type==='tbill'){
      const avg=v.reduce((s,x)=>s+Number(x.value),0)/v.length;
      const days=Math.max(1,(new Date(end)-new Date(start))/86400000);
      return {value:avg*days/365,start,end,unit:v[v.length-1].unit,calculated:true};
    }
    return null;
  };

  async function horizonEvidence(h){
    const r=await get('/rest/v1/fund_performance_history?select=report_date&horizon=eq.'+encodeURIComponent(h)+'&order=report_date.asc&limit=1000');
