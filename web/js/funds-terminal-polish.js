...
function addSignal(row,index,total){
  ...
  signal.className='row-signal '+cls;
  signal.innerHTML='<i class="signal-dot"></i><strong>'+label+'</strong>';
  if(cls==='warning')row.classList.add('terminal-row-highlight');else row.classList.remove('terminal-row-highlight');
  if(scoreCell.dataset.polished==='1')return;
  ...
  if(retCell.querySelector('.return-sub'))return;
  const sub=document.createElement('span');sub.className='return-sub';sub.textContent=row.querySelector('.report-date')?.textContent||'العائد الرسمي';retCell.appendChild(sub);
  if(qual.dataset.polished==='1')return;qual.dataset.polished='1';const txt=q;qual.innerHTML='<span class="qual-pill '+(/مؤهل/.test(txt)&&!(/غير|تحت/.test(txt))?'good':/غير|تحت/.test(txt)?'bad':'')+'">'+txt+'</span>';
...
const obs=new MutationObserver(run);obs.observe(document.documentElement,{subtree:true,childList:true});
