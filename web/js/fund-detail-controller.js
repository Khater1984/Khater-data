...
(function (global) {
  ...
  async function renderMacroContext(F, fund) {
    ...
    host.innerHTML = '<section class="decision-card" aria-label="السياق السوقي للصندوق"><div class="decision-kicker">MARKET CONTEXT · FUND DNA</div><div class="decision-main"><div><h2>' + F.esc(regime) + '</h2><p>' + F.esc(copy) + '</p><p class="decision-category-note">الفئة: <strong>' + F.esc(fund.category || 'غير محددة') + '</strong> · لا توجد توصية شراء أو بيع.</p></div><div class="decision-score mid"><span>CONTEXT</span><b>LIVE</b><small>Macro + Fund</small></div></div><div class="decision-grid macro-context-grid">' + metrics + '</div><div class="decision-path"><b>Macro</b><i>→</i><b>Category</b><i>→</i><b>Benchmark</b><i>→</i><b>Fund</b></div></section>';
  }
  ...
  function render(F, app, d) {
    ...
    app.innerHTML = '<section class="hero hero-pro"><div class="hero-price-panel" aria-label="سعر الوثيقة"><div class="hero-price-label">سعر الوثيقة</div><div class="hero-price-value">' + navValue + ' <span>' + F.esc(navCurrency) + '</span></div><div class="hero-price-meta">آخر تحديث: ' + F.esc(navDate) + '</div><a class="hero-price-compare" href="./funds.html?compare=' + encodeURIComponent(F.id) + '">قارن</a></div>' + logoBox(F, profile.manager_logo_url, manager) + '<div class="hero-copy"><div class="eyebrow">FUND DNA · INSTITUTIONAL PROFILE</div><h1>' + F.esc(f.canonical_name) + '</h1><div class="manager">' + F.esc(manager) + '</div><div class="tags"><span class="tag">' + F.esc(f.category) + '</span><span class="tag">' + F.esc(s.data_tier || '—') + '</span><span class="tag">' + F.esc(s.methodology_version || '—') + '</span></div></div></section>' +
      '<section class="pulse"><div class="metric primary"><div class="muted">SMART SCORE</div><span class="metric-value">' + score + '</span><span class="pill">' + F.esc(s.rating || 'NOT RATED') + '</span></div><div class="metric"><div class="muted">RETURN · ' + F.esc(F.L[h] || h) + '</div><span class="metric-value ' + retClass + '">' + heroReturn + '</span><div class="note">' + F.esc(heroDate) + '</div></div><div class="metric"><div class="muted">DATA CONFIDENCE</div><span class="metric-value metric-compact">' + F.esc(s.data_confidence || '—') + '</span><div class="note">' + F.esc(s.data_quality || '') + '</div></div><div class="metric"><div class="muted">QUALIFICATION</div><span class="metric-value metric-qualification">' + F.esc(s.qualification_status || '—') + '</span><div class="note">Track Factor · ' + (s.track_factor == null ? '—' : Number(s.track_factor).toFixed(2)) + '</div></div></section>' +
      '<div id="fund-market-context"></div><div id="decision-layer"></div>' +
      '<div class="tabs" role="tablist" aria-label="Fund DNA analytical sections"><button id="tab-performance" class="active" role="tab" aria-selected="true" aria-controls="performance-panel" data-tab="performance"><b>01</b><span>PERFORMANCE</span><small>العائد والحركة</small></button><button id="tab-risk" role="tab" aria-selected="false" aria-controls="risk-panel" data-tab="risk"><b>02</b><span>RISK</span><small>مؤشر المخاطر</small></button><button id="tab-benchmark" role="tab" aria-selected="false" aria-controls="benchmark-panel" data-tab="benchmark"><b>03</b><span>BENCHMARK</span><small>مقارنة السوق</small></button><button id="tab-smartscore" role="tab" aria-selected="false" aria-controls="smartscore-panel" data-tab="smartscore"><b>04</b><span>SMARTSCORE</span><small>مكونات التقييم</small></button><button id="tab-evidence" role="tab" aria-selected="false" aria-controls="evidence-panel" data-tab="evidence"><b>05</b><span>EVIDENCE</span><small>الدليل والمنهج</small></button><button id="tab-profile" role="tab" aria-selected="false" aria-controls="profile-panel" data-tab="profile"><b>06</b><span>FUND PROFILE</span><small>بيانات الصندوق</small></button></div>' +
      ...
      '<section id="performance-panel" class="tab-panel active" role="tabpanel" aria-labelledby="tab-performance"><div id="performance-tab"></div></section><section id="risk-panel" class="tab-panel" role="tabpanel" aria-labelledby="tab-risk"><div id="risk-tab"></div></section><section id="benchmark-panel" class="tab-panel" role="tabpanel" aria-labelledby="tab-benchmark"><div id="benchmark-tab"></div></section><section id="smartscore-panel" class="tab-panel" role="tabpanel" aria-labelledby="tab-smartscore"><div id="smartscore-tab"></div></section><section id="evidence-panel" class="tab-panel" role="tabpanel" aria-labelledby="tab-evidence"><div id="evidence-tab"></div></section><section id="profile-panel" class="tab-panel" role="tabpanel" aria-labelledby="tab-profile"><div id="profile-tab"></div></section>';
  }
